...
class Motor_Control:
    ...
    def map_input(self,corr_param):
        f = 5*(math.log10(1+corr_param))
	f = (int(f))
        if f > 30 :
            return 30
        return f
    ...
    def control_motion_pwm(self,corr_param,corr_direc):
        ...
        if Motor_Control.PWM == True and len(Motor_Control.res) == 4 :
            ...
            if Motor_Control.boolean_op:
                if corr_direc > 0:
                    Motor_Control.res[0].ChangeDutyCycle(100 - vel_diff)
                    Motor_Control.res[2].ChangeDutyCycle(100)
		    print 'Left '
                else:
                    Motor_Control.res[0].ChangeDutyCycle(100)
                    Motor_Control.res[2].ChangeDutyCycle(100 - vel_diff)
		    print 'Right '
            else:
                if corr_direc > 0:
                    Motor_Control.res[0].start(100 - vel_diff)
                    Motor_Control.res[2].start(100)
		    print 'Left '
                else:
                    Motor_Control.res[0].start(100)
                    Motor_Control.res[2].start(100 - vel_diff)
		    print 'Right '
    ...
    def __init__(self,p):
        ...
        if self.setup_pin(Motor_Control.pin):
            Motor_Control.res = self.set_PWM(Motor_Control.pin,100)
            '''
            while True:
                val = int(raw_input('Enter val No. '))
                dire = int(raw_input('Enter direc No. '))
                if val == 0:
                    break
                else :
                    self.control_motion_pwm(val,dire)
            '''
            print len(Motor_Control.res)
